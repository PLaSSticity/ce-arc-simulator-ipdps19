...
public class Knobs {
	...
	// The following choices are valid for ARC as well
	...
	static {
		...
		WritebackInMemory = registerBool(
				parser.accepts("writeback-in-memory", "Writeback metadata in memory")
						.withOptionalArg().ofType(Boolean.class).defaultsTo(false));

		AlwaysInvalidateReadOnlyLines = registerBool(parser
				.accepts("always-invalidate-read-only-lines", "Self invalidate read-only lines")
				.withRequiredArg().ofType(Boolean.class).defaultsTo(true));
		...
		UpdateWrittenLinesDuringVersionCheck = registerBool(parser
				.accepts("update-written-lines-during-version-check",
		...
		InvalidateUntouchedLinesOptimization = registerBool(parser
				.accepts("invalidate-untouched-lines-opt",
						"Check version of untouched lines before self-invalidating")

				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));

		UseSpecialInvalidState = registerBool(parser
				.accepts("special-invalid-state",
						"Use special invalid state, subject to version check")
				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));
		UseBloomFilter = registerBool(
				parser.accepts("use-bloom-filter", "Use bloom filter to store updated LLC lines.")
						.withRequiredArg().ofType(Boolean.class).defaultsTo(false));
		UseTwoBloomFuncs = registerBool(
				parser.accepts("use-two-bloom-funcs", "Use two Bloom filter functions.")
						.withRequiredArg().ofType(Boolean.class).defaultsTo(true));
		...
		DeferWritebacks = registerBool(parser
				.accepts("defer-write-backs",
						"Defer write backs of dirty lines at line granularity")
				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));
		DeferredWritebacksPrecise = registerBool(parser
				.accepts("defer-write-backs-precise",
						"Defer write backs of dirty lines precisely (at byte granularity")
				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));

		SkipValidatingReadLines = registerBool(parser
				.accepts("skip-validating-read-lines",
						"Skip validating read lines if they are not updated in the LLC")
				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));

		IgnoreFetchingDeferredLinesDuringReadValidation = registerBool(parser
				.accepts("ignore-deferred-lines-read-validation",
						"Incorrectly ignore fetching deferred lines "
								+ "during read validation to see the impact")
				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));
		IgnoreFetchingReadBits = registerBool(parser
				.accepts("ignore-fetching-read-bits",
						"Ignore fetching read access bits into private caches")
				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));
		IgnoreFetchingWriteBits = registerBool(parser
				.accepts("ignore-fetching-write-bits",
						"Ignore fetching write access bits into private caches")
				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));
		...
		SiteTracking = registerBool(
				parser.accepts("site-tracking", "Track site information for memory accesses.")
						.withRequiredArg().ofType(Boolean.class).defaultsTo(false));
		...
		TreatAtomicUpdatesAsRegularAccesses = registerBool(parser
				...
				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));
		IsHttpd = registerBool(parser.accepts("is-httpd", "Simulate the Apache Httpd server")
				.withRequiredArg().ofType(Boolean.class).defaultsTo(false));
		...
		TreatAtomicUpdatesAsRegionBoundaries = registerBool(parser
				.accepts("treat-atomic-updates-as-region-boundaries",
