...
class MemoryResponse<Line extends ViserLine> {
	...
	// implies line was in special invalid state, but version check failed
	public boolean invalidStateFailure;
	public CacheLevel invalidStateSharedHitLevel; // applicable to special invalid case

	/** Track AIM hit (LLC) or miss (MEMORY). */
	public CacheLevel aimHit;
...
public class HierarchicalCache<Line extends ViserLine> {
	...
	/**
	 * The next higher-level cache in the hierarchy. Can be shared by multiple lower-level caches.
	 */
	...
	/**
	 * Construct a component of the cache hierarchy. NB: the components must be constructed in
	...
	/**
	 * Update LLC line with updated write encoding, epoch, and values. Note that the corresponding
	 * LLC line might have been evicted by this time.
	 */
	boolean updateWriteInfoInLLC(Processor<Line> proc, Line privLine, ExecutionPhase phase) {
		assert privLine
				.hasWrittenOffsets(proc.id) : "Private line is expected to have write metadata.";
		...
		// The line is not in the LLC, possibly it was evicted from the LLC, but not from L1/L2
		// since they are not inclusive. Bring in the line from memory. This is a policy decision,
		// we could have avoided it, but it might be beneficial since we might use the line again
		// during read validation.
		if (llcLine == null) {
			hit = false;

			Line tmp = proc.machine.memory.get(privLine.lineAddress().get());
			assert tmp.getLevel() == CacheLevel.L3 && tmp.id().equals(processor.id)
					&& !tmp.isLineDeferred();
			...
			if (!proc.params.writebackInMemory()) {
				...
				if (toEvict.valid()) {
					...
					if (proc.params.useAIMCache()
							&& /* proc.needToCheckAIMCache(toEvict) */ toEvict.hasAIMMD()) {
						proc.aimcache.evictLine(proc, toEvict, phase);
					}
				}
			}
		}
		...
		// Check if the LLC line is "already" deferred by a different core, if yes, then get the
		// values from the owner core
		if (proc.params.deferWriteBacks()) {
			if (llcLine.getVersion() != privLine.getVersion() ||
			// the following is for L2 lines whose versions have been updated during validation
				...
				if (proc.params.areDeferredWriteBacksPrecise()) {
					sizeBytes += (SystemConstants.VISER_WRITE_METADATA_BYTES
							+ Long.bitCount(privLine.getWriteEncoding(cid)));
				} else {
					// only send dirty values
					sizeBytes += SystemConstants.DATA_MESSAGE_CONTROL_BYTES
							+ Long.bitCount(privLine.getWriteEncoding(cid));
					// + MemorySystemConstants.DATA_MESSAGE_SIZE_BYTES);
				}
				proc.updateTrafficForOneNetworkMessage(1, sizeBytes, ExecutionPhase.POST_COMMIT);
				...
				if (proc.params.areDeferredWriteBacksPrecise()) {
					...
					Long newWrMd = new Long(privLine.getWriteEncoding(cid));
					// Check whether the line is already deferred, in that case need to OR the write
					// metadata
					if (existingWrMd != null) {
						assert Long.bitCount(
								existingWrMd) > 0 : "Otherwise it should not have been deferred";
		...
		// For site tracking, set last writers even though the actual values are deferred.
		llcLine.setLastWritersFromPrivateLine(privLine);
		llcLine.orWriteEncoding(cid, privLine.getWriteEncoding(cid));
		llcLine.updateWriteSiteInfo(cid, privLine.getWriteEncoding(cid),
				privLine.getWriteSiteInfo(cid), privLine.getWriteLastSiteInfo(cid));
		...
		if (proc.params.useAIMCache()) {
			if (privLine.hasReadOffsets(cid) || privLine.hasWrittenOffsets(cid)) {
				llcLine.setAIMMD(true);
				// The line might be in the AIM cache
				proc.aimcache.addLineIfNotPresent(proc, privLine, false, true, phase);
			}
		}
		return hit;
	}
	...
	private boolean updateReadInfoInLLC(Processor<Line> proc, Line privLine, ExecutionPhase phase) {
		...
		if (llcLine == null) {
			...
			if (!proc.params.writebackInMemory()) {
				...
				if (toEvict.valid()) {
					evictValidLineFromLLC(proc, toEvict, phase);
					if (proc.params.useAIMCache() && /* proc.needToCheckAIMCache(toEvict) */
							toEvict.hasAIMMD()) {
						proc.aimcache.evictLine(proc, toEvict, phase);
					}
				}
		...
		llcLine.updateReadSiteInfo(cid, privLine.getReadEncoding(cid),
				privLine.getReadSiteInfo(cid), privLine.getReadLastSiteInfo(cid));
	...
	// We include network traffic costs in the callers.
	// Only check conflicts for written lines while making no changes in the LLC except for
	// fetching the line from the memory if missing
	boolean checkConflictsForWrittenLines(Processor<Line> proc, Line privLine,
		...
		if (proc.params.useAIMCache()) {
			if (privLine.hasReadOffsets(proc.id) || privLine.hasWrittenOffsets(proc.id)) {
				llcLine.setAIMMD(true);
				// The line might be in the AIM cache
				proc.aimcache.addLineIfNotPresent(proc, privLine, true, true, phase);
			}
		}

		// No need to pre-validate WAR lines since we send values after performReadValidation().
		return hit;
	}
	...
	void handleWriteAfterReadUpgrade(Line l1Line, long enc) {
		...
		// Encoding updates should comply with value updates and be only for those involved in a WAR
		// upgrade
		l2Line.orReadEncoding(l1Line.id(), enc);
		// updateSourceLocationInfo(l2Line, l1Line, processor.id, true);// !! wrong: only those
		// offsets which have been
		// accessed in enc should be updated
		CpuId cid = l1Line.id();
		l2Line.updateReadSiteInfo(cid, enc, l1Line.getReadSiteInfo(cid),
				l1Line.getReadLastSiteInfo(cid));
		// There is no on-chip communication since this is between private L1 and L2 caches.
	}

	/**
	 * Private L1 and L2 caches are inclusive. This implies a recall if a valid line is evicted from
	 * L2. Note that the L2 cache is larger than L1, so the cache line may not be present in L1 at
	 * the time of recall.
	 */
	Line recallFromL1Cache(CacheLevel level, Line toEvict) {
		assert (level == CacheLevel.L2)
				&& (levelInHierarchy == CacheLevel.L1) : "Only L1 and L2 are inclusive.";
	...
	// Virtualize a valid line that is getting evicted from the LLC. The LLC and the private L1/L2
	...
	private void evictValidLineFromLLC(Processor<Line> proc, Line toEvict, ExecutionPhase phase) {
		...
		// Check if the line is deferred, if yes, then get the values from the owner core
		...
		int numReadMdWritebacks = 0;
		int numWriteMdWritebacks = 0;
		...
		for (int i = 0; i < proc.params.numProcessors(); i++) {
			...
			// Write back read metadata only if the region is ongoing. Write back the write metadata
			// if the region is ongoing. Write back values if the line is dirty.
			if (md.epoch.equals(p.getCurrentEpoch())) { /* The region is ongoing */
				// The private line metadata may not have been written back to the LLC, since we do
				// not have inclusivity.
				// assert toEvict.hasReadOffsets(cpuId) || toEvict.hasWrittenOffsets(cpuId);

				// RCC need not write back a line unless it has valid metadata bits or is dirty. The
				// fact that a
				// private cache has a line cached in an ongoing core is not sufficient reason to
				// write back the line.
				...
				if (toEvict.hasWrittenOffsets(cpuId)) {
					mdWritebackNeeded = true;
					numWriteMdWritebacks++;
				}
			}
		}

		if (mdWritebackNeeded || toEvict.dirty()) {
			// We ignore the one way cost for execution cycles
			int sizeBytes = SystemConstants.TAG_BYTES + proc.machine.VISER_VARIABLE_MSG_HEADER +
			/* read encoding */ (numReadMdWritebacks * SystemConstants.VISER_READ_METADATA_BYTES) +
			/* write encoding */ (numWriteMdWritebacks
					* SystemConstants.VISER_WRITE_METADATA_BYTES);
			if (toEvict.dirty()) {
				sizeBytes += SystemConstants.DATA_MESSAGE_SIZE_BYTES; // values
				sizeBytes += SystemConstants.VISER_VERSION_BYTES; // version
			}
			proc.updateTrafficForOneNetworkMessage(1, sizeBytes, phase);
			// proc.updateTrafficForLLCToMemoryMessage(sizeBytes, false);
			// proc.updateMemoryAccesses(sizeBytes, false);
		...
		// Since a line is getting evicted from the LLC, we can safely overwrite the contents in
		// memory (if present). We
		// always write back so that we can always assert that a line will either be in the LLC or
		// in memory. This is a source of space overhead.
	...
	private void updateL2LineWithL1Line(Processor<Line> proc, Line destLine, Line sourceLine,
			boolean isL1Eviction) {
		assert destLine.valid() && sourceLine.valid();
		assert destLine.getVersion() == sourceLine
				.getVersion() : "L1 and L2 line versions are read-only.";
		...
		if (sourceLine.hasReadOffsets(cid)) {
			...
			destLine.updateReadSiteInfo(cid, sourceLine.getReadEncoding(cid),
					sourceLine.getReadSiteInfo(cid), sourceLine.getReadLastSiteInfo(cid));
		}

		if (isL1Eviction) {
			if (sourceLine.hasWrittenOffsets(cid)) {
				setMRUBit(destLine, false); // Ensure the invariance that we set MRU bits every time
											// setting write bits
			}
		} else if (!destLine.hasWrittenOffsets(cid) && sourceLine.hasWrittenOffsets(cid)) {
		...
		destLine.updateWriteSiteInfo(cid, sourceLine.getWriteEncoding(cid),
				sourceLine.getWriteSiteInfo(cid), sourceLine.getWriteLastSiteInfo(cid));
		/*
		 * With ARC's optimizations, even untouched offsets may have values which haven't been
		 * updated to the LLC due to deferring write-backs or have been updated to the LLC but not
		 * to the L2. In either case, they shouldn't be lost (in the second case, if the values are
		 * not updated to the L2, following reads will read obsolete values from the L2 and cause
		 * read validation failure). So we need to copyAllValues from the L1 line to the L2 line
		 * here.
		 */
	...
	protected void evictedFromHigherCache(Processor<Line> proc, Line incoming,
			ExecutionPhase phase) {
	...
	private boolean validateReadL2LineOnL1Eviction(Processor<Line> proc, Line l1Line, Line l2Line,
		...
		// Check for version mismatch, do precise write-read conflict checking if no conflict, then
		// do value validation. If versions match and write bit is set in the shared line, check for
		// precise conflicts

		// The following doesn't fetch the line from memory to LLC even if a LLC miss
		// AIM stats should be updated in the call to updateReadInfoInLLC()
		MemoryResponse<Line> resp = proc.L3cache.requestLineFromLLCOrMemory(proc, l2Line, true,
				phase);
		assert resp.lineHit != null;
		Line sharedLine = resp.lineHit;

		if (proc.params.validateL1ReadsAlongWithL2()) {
			l2Line.orReadEncoding(proc.id,
					l1Line.getReadEncoding(proc.id) & ~l1Line.getWriteEncoding(proc.id));
			l2Line.copyReadOnlyValuesFromSource(l1Line);
			l2Line.updateReadSiteInfo(proc.id,
					l1Line.getReadEncoding(proc.id) & ~l1Line.getWriteEncoding(proc.id),
					l1Line.getReadSiteInfo(proc.id), l1Line.getReadLastSiteInfo(proc.id));
		}

		if (!proc.matchVersions(l2Line, sharedLine)) {
			// might save the cost of value validation, but introduce false validation failure when
			// values match
			...
			// LLC responds with a NACK, core responds with values
			int sizeBytesIncoming = SystemConstants.CONTROL_MESSAGE_SIZE_BYTES;
			proc.updateTrafficForOneNetworkMessage(1, sizeBytesIncoming,
					ExecutionPhase.REGION_BODY);
			proc.updateUpgradeTrafficForOneNetworkMessage(1, sizeBytesIncoming);

			int sizeBytesReturn = SystemConstants.DATA_MESSAGE_CONTROL_BYTES;
			// All read bytes need to be validated, not read-only
			long readBits = l2Line
					.getReadEncoding(proc.id) /* & ~l2Line.getWriteEncoding(proc.id) */;
			sizeBytesReturn += Long.bitCount(readBits);
			proc.updateTrafficForOneNetworkMessage(1, sizeBytesReturn, ExecutionPhase.REGION_BODY);
			proc.updateUpgradeTrafficForOneNetworkMessage(1, sizeBytesReturn);

			if (proc.valueValidateReadLine(ExecutionPhase.EVICT_L1_READ_VALIDATION, l2Line,
					sharedLine)) {
			...
			if (proc.checkIfWriteBitIsSet(sharedLine)) {
				proc.checkPreciseWriteReadConflicts(sharedLine, l2Line,
						ExecutionPhase.EVICT_L1_READ_VALIDATION);
			}
		}
	 ...
	 */
	private void evictedFromL1Cache(Processor<Line> proc, Line incomingL1Line,
			ExecutionPhase phase) {
		// search this L2 cache
		Line l2Line = getLine(incomingL1Line);
		assert l2Line != null : "L1 and L2 are inclusive";
		assert l2Line.getVersion() == incomingL1Line
				.getVersion() : "L1 and L2 versions should match";
		...
		if (incomingL1Line.isWrittenAfterRead(incomingL1Line.id())) {
			boolean hit = validateReadL2LineOnL1Eviction(proc, incomingL1Line, l2Line, phase);

			// Send read encoding to the LLC for an upgrade so as to eagerly catch read-write
			// conflicts
			// Moved into validateReadL2LineOnL1Eviction.
			// We are not counting the execution cost, since this is not on the critical path
			// This generates traffic
			int sizeBytes = SystemConstants.DATA_MESSAGE_CONTROL_BYTES + SystemConstants.TAG_BYTES
					+ SystemConstants.VISER_READ_METADATA_BYTES +
					// Need to send versions to catch conflicts with writer cores that have
					// completed their regions
					SystemConstants.VISER_VERSION_BYTES;
			proc.updateUpgradeTrafficForOneNetworkMessage(1, sizeBytes);
			if (l2Line.getVersion() < SystemConstants.MAX_14_BIT_VERSION) {
				proc.updateTrafficForOneNetworkMessage(1,
						sizeBytes - SystemConstants.VISER_VERSION_BYTES, phase);
			} else {
				proc.updateTrafficForOneNetworkMessage(1, sizeBytes, phase);
			}
			proc.updateVersionSizeHistogram(l2Line.getVersion());

			if (!hit) { // Check if it is an LLC miss
				proc.updateTrafficForOneNetworkMessage(1, sizeBytes, phase);
				// proc.updateTrafficForLLCToMemoryMessage(sizeBytes, false); // Update in memory
				// proc.updateMemoryAccesses(sizeBytes, false);
			}
		}
	 ...
	 */
	private void evictedFromL2Cache(Processor<Line> proc, Line incomingL2Line,
			ExecutionPhase phase) {
		// search this cache
		...
		if (sharedLine == null) {
			...
			assert sharedLine.id().get() == 0 && sharedLine.getLevel() == CacheLevel.L3
					&& !sharedLine.isLineDeferred();
		...
		// WAR upgraded offsets have already been taken care of, this only needs to take care of
		// read-only offsets
		...
		if (incomingL2Line.hasReadOnlyOffsets(proc.id)) {
			...
			if (!proc.matchVersions(incomingL2Line, sharedLine)) {
				...
				// might save the cost of value validation, but introduce false validation failure
				// when values match
				...
				proc.updateTrafficForOneNetworkMessage(1, sizeBytesOutgoing,
						ExecutionPhase.REGION_BODY);
				proc.updateUpgradeTrafficForOneNetworkMessage(1, sizeBytesOutgoing);

				int sizeBytesReturn = SystemConstants.DATA_MESSAGE_CONTROL_BYTES;
				long readOnlyBits = incomingL2Line.getReadEncoding(proc.id)
						& ~incomingL2Line.getWriteEncoding(proc.id);
				sizeBytesReturn += Long.bitCount(readOnlyBits);
				proc.updateTrafficForOneNetworkMessage(1, sizeBytesReturn,
						ExecutionPhase.REGION_BODY);
				proc.updateUpgradeTrafficForOneNetworkMessage(1, sizeBytesReturn);

				if (proc.valueValidateReadLine(ExecutionPhase.EVICT_L2_READ_VALIDATION,
						incomingL2Line, sharedLine)) {
					proc.stats.pc_FailedValidations.incr();
					if (!proc.regionWithExceptions) {
						proc.stats.pc_RegionsWithFRVs.incr();
						if (!proc.regionHasDirtyEvictionBeforeFRV && proc.hasDirtyEviction) {
		...
		assert md.epoch.getRegionId() <= p.getCurrentEpoch()
				.getRegionId() : "LLC line epoch cannot be greater.";
		// Update read bits
		sharedLine.orReadEncoding(cid, incomingL2Line.getReadEncoding(cid));
		sharedLine.updateReadSiteInfo(cid, incomingL2Line.getReadEncoding(cid),
				incomingL2Line.getReadSiteInfo(cid), incomingL2Line.getReadLastSiteInfo(cid));
		// Update write bits
		// orWriteEncoding should be performed to clear obsolete encoding even if the incoming line
		// has no written offsets
		sharedLine.orWriteEncoding(cid, incomingL2Line.getWriteEncoding(cid));
		sharedLine.updateWriteSiteInfo(cid, incomingL2Line.getWriteEncoding(cid),
				incomingL2Line.getWriteSiteInfo(cid), incomingL2Line.getWriteLastSiteInfo(cid));
		...
		if (status == DeferredStatus.NOT_DEFERRED || status == DeferredStatus.REMOTE_CORE) {
			...
			if (proc.params.areDeferredWriteBacksPrecise()) {
				...
				if (proc.params.areDeferredWriteBacksPrecise()) {
					...
					assert Long
							.bitCount(writeMd) > 0 : "Otherwise it should not have been deferred";
					proc.removeDeferredWriteLine(incomingL2Line);
				}
			}
			// Need to send all values into the LLC, because possibly they are deferred written
		...
		// The lock/atomic line is already in the LLC and the LLC doesn't need to keep the ownerID
		// to communicate with the owner core.
		if (sharedLine.getLockOwnerID() == proc.id.get())
			sharedLine.clearLockOwnerID();
		// obsolete encodings have been cleared by orEncoding() before setEpoch(); setEpoch() before
		// getEncoding()
		sharedLine.setEpoch(cid, p.getCurrentEpoch());

		// If both read and write offsets are set in the shared line, then no need to maintain read
		// encoding.
		// Reset read encoding in the LLC line directly. The LLC might have both read and write bits
		...
		if (proc.params.useAIMCache()) {
			if (incomingL2Line.hasReadOffsets(cid) || incomingL2Line.hasWrittenOffsets(cid)) {
				sharedLine.setAIMMD(true);
				// The line might be in the AIM cache
				proc.aimcache.addLineIfNotPresent(proc, incomingL2Line, false, true, phase);
			}
		}
		...
		// Compute network traffic stats
		int sizeBytes = SystemConstants.DATA_MESSAGE_CONTROL_BYTES + SystemConstants.TAG_BYTES;
		// If offsets are written after read, then we avoid writing them back since
		// they are not useful for conflict detection.
		if (incomingL2Line.hasReadOnlyOffsets(cid)) {
			sizeBytes += SystemConstants.VISER_READ_METADATA_BYTES;
			sizeBytes += SystemConstants.VISER_VERSION_BYTES; // needed for validation
		}
		if (status == DeferredStatus.SAME_CORE) {
			sizeBytes += SystemConstants.DATA_MESSAGE_SIZE_BYTES;
		} else {
			long writeEnc = incomingL2Line.getWriteEncoding(cid);
			sizeBytes += Long.bitCount(writeEnc);
		}
		if (incomingL2Line.hasWrittenOffsets(cid)) {
			sizeBytes += SystemConstants.VISER_WRITE_METADATA_BYTES;
		}
		...
		if (sharedLine.getVersion() <= SystemConstants.MAX_14_BIT_VERSION) {
			proc.updateTrafficForOneNetworkMessage(1,
					sizeBytes - SystemConstants.VISER_VERSION_BYTES, phase);
		} else {
			proc.updateTrafficForOneNetworkMessage(1, sizeBytes, phase);
		...
		// Account for TCC traffic
		proc.updateTCCBroadcastMessage(sizeBytes - SystemConstants.VISER_VERSION_BYTES);
	}

	/**
	 * Search for a valid line {@code incoming} in the private L1 cache and return true. We ignore
	 * L2 for now.
	 */
	...
	MemoryResponse<Line> requestLineFromLLCOrMemory(Processor<Line> proc, Line privLine,
			boolean read, ExecutionPhase phase) {
		int setIndex = index(privLine.lineAddress().get());
		...
		for (Iterator<Line> it = set.iterator(); it.hasNext();) {
			Line line = it.next();
			if (line.valid() && line.lineAddress().equals(privLine.lineAddress())) {
				// hit!
				if (proc.params.usePLRU()) {
					setMRUBit(line, read);
				} else {
				...
				if (proc.params.useAIMCache()) {
					if (/* proc.needToCheckAIMCache(line) */line.hasAIMMD()) {
						AIMResponse<Line> aimResp = proc.aimcache.request(proc, privLine, read,
								phase);
						ret.aimHit = aimResp.whereHit;
					} else {
						ret.aimHit = CacheLevel.L3;
					}
				}
		...
		// if we made it here, we missed in LLC

		// bring line from memory, this line needs to be owned by P0
		Line memLine = processor.machine.memory.get(privLine.lineAddress().get());
		ret.whereHit = CacheLevel.MEMORY;

		// This is being returned, on behalf of the current processor, P0
		if (memLine == null) {
			memLine = lineFactory.create(processor, levelInHierarchy, privLine.lineAddress());
			memLine.changeStateTo(ViserState.VISER_VALID);
		} else {
			assert !memLine.isLineDeferred();
			memLine = proc.clearAccessEncoding(memLine); // memLine may get modified
		}

		// proc.accountOffChipTraffic(memLine); // return data message
		// int sizeInBytes = SystemConstants.DATA_MESSAGE_CONTROL_BYTES
		// + SystemConstants.DATA_MESSAGE_SIZE_BYTES + proc.getAIMLineSize();
		// proc.updateMemoryAccesses(sizeInBytes, true);
		...
		assert levelInHierarchy == CacheLevel.L3 : "Impossible cache level: MEMORY";
		assert ret.whereHit == CacheLevel.MEMORY && memLine != null;
		assert ret.lineHit != null && ret.lineHit.valid();

		Line toEvict = callbacks.eviction(ret.lineHit, set, levelInHierarchy, phase,
				MRUBits[setIndex]);
		...
		if (proc.params.usePLRU()) {
			...
			// This is not required for correctness, conflicts on private cache lines will be
			// detected lazily.
		...
		// So we just fetched a line into the LLC, create one in the AIM cache.
		if (proc.params.useAIMCache()) {
			proc.aimcache.addLineWithoutCheckingForDuplicates(proc, memLine, read,
					/* proc.needToCheckAIMCache(memLine) */ memLine.hasAIMMD(), phase);
		}

		return ret;
	}

	MemoryResponse<Line> requestWithSpecialInvalidState(Processor<Line> proc,
			final DataAccess access, boolean read) {
		...
		for (Iterator<Line> it = set.iterator(); it.hasNext();) {
			...
			if (line.valid() && line.contains(address)) {
				...
				if (line.getState() == ViserState.VISER_INVALID_TENTATIVE) {
					...
					// Check if the line still contains valid values by comparing versions with
					// shared memory
					MemoryResponse<Line> sharedResp = proc.getLineFromLLCOrMemory(line, read);
				...
				if (levelInHierarchy == CacheLevel.L1) {
					...
					if (proc.params.useAIMCache()) {
						if (/* proc.needToCheckAIMCache(line) */ line.hasAIMMD()) {
							// Check if AIM hit/miss
							AIMResponse<Line> aimResp = proc.aimcache.request(proc, line, read,
									phase);
							ret.aimHit = aimResp.whereHit;
						} else {
							ret.aimHit = CacheLevel.L3;
		...
		if (levelInHierarchy == CacheLevel.L1) { // miss in L1
			...
			if (nextCache != null) { // Valid check is in the callee
				nextCache.evictedFromHigherCache(proc, toEvict, phase);
			}

			// Create a copy of the line, not required since ret.lineHit already represents an
			// unique copy, but
			// we want to set the correct owner processor
			Line insert = lineFactory.create(proc, levelInHierarchy, ret.lineHit);
			// remove the evicted line and insert the new line only after a successful eviction
			if (proc.params.usePLRU()) {
				int lineIndex = set.indexOf(toEvict);
			...
			// Before L2 can evict a line, it has to fetch updated access information from the L1.
			// Check if the same line is in higher-level caches. If yes, then the line should
			// also be evicted from all higher-level caches.
			if (toEvict.valid()) {
				// Set the epoch for a line before being evicted from the L2 cache. The following
				// assertion
				...
				if (l1Line != null) {
					...
					if (l1Line.isWrittenAfterRead(l1Line.id())) {
						boolean hit = validateReadL2LineOnL1Eviction(proc, l1Line, toEvict, phase);

						// Send read encoding to the LLC for an upgrade, to eagerly detect
						// read-write conflicts
						// moved into validateReadL2LineOnL1Eviction();
						// This generates traffic, but we do not account it over here. Mike suggests
						// that we can club
						// this message with the write that we will do next, as part of the write
						// back of the line to the
						// LLC. But we continue to account for the off-chip memory traffic.
			...
			// If we come here, it means we were required to fetch a line from memory because of an
			// LLC miss.
			assert ret.whereHit == CacheLevel.MEMORY && memLine != null;
			assert ret.lineHit != null && ret.lineHit.valid();
			...
			// So we just fetched a line into the LLC, create one in the AIM cache
			if (proc.params.useAIMCache()) {
				proc.aimcache.addLineFromLLCMiss(proc, memLine, read, phase);
			}
		} else {
		...
		if ((bits & assocMask) == assocMask) { // all bits are set
			if (processor.params.evictCleanLineFirst() && levelInHierarchy == CacheLevel.L2
				...
				for (int i = 0; i < set.size(); i++) {
					...
					boolean isDirty = line.hasWrittenOffsets(cid);
			...
			if ((bits & assocMask) == assocMask || (bits & assocMask) == 0) { // all lines in the
																				// set are
																				// dirty/clean
