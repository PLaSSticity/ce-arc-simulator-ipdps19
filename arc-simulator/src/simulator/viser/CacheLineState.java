...
class VILine extends CoherentLine {
  ...
  public String toString() {
    return "addr=" + (addr == null ? "null" : addr.toString()) + "\t"
        + (valid ? "valid" : " invalid") + "\t" + (dirty() ? " dirty" : " clean");
  }
...
class ViserLine extends VILine {
  ...
  /**
   * Used for validation, and to prevent ABA problem with value validation. Incremented on every
   * writeback to LLC.
   */
  private int version = 0;

  /**
   * Viser needs to store values in addition to tags (unlike MESI). We do not need per-core values,
   * since LLC values are globally visible.
   */
  private long[] values = new long[SystemConstants.LINE_SIZE()];
  ...
  /**
   * LLC needs to maintain the owner core in case write back of dirty lines are deferred.
   */
  ...
  /**
   * Does the AIM line have valid metadata. For a new line, this can be set when evicting from a
   * private cache or when fetching from the memory data structure.
   */
  private boolean hasAIMMD;
  ...
  @Override
  public String toString() {
    String str = super.toString() + "\t" + this.state.toString() + "\tIdHash:"
        + System.identityHashCode(this) + "\tCPU:" + id() + "\tLevel:" + level + "\tVer:" + version
        + "\tLock owner:" + lockOwnerID + "\tAIM MD:" + hasAIMMD;
    if (isPrivateCacheLine()) {
      str += "\tRegion:" + getEpoch(id()) + "\tWrite bits:"
          + Long.toHexString(getWriteEncoding(id())) + "\tRead bits:"
          + Long.toHexString(getReadEncoding(id()));
    } else if (ownerProc.params.deferWriteBacks()) {
      str += "\tOwnerCore:" + deferredLineOwnerID;
    }
    str += "\t";
    /*
     * for (int i = 0; i < values.length; i++) { str = str + "[" + i + "]:" + values[i] + " "; }
     */
    return str;
  }
  ...
  public void incrementVersion() {
    ...
    assert Integer.MAX_VALUE == Math.pow(2,
        ((SystemConstants.VISER_VERSION_BYTES * SystemConstants.BITS_IN_BYTE) - 1)) - 1;
    if (version > Math.pow(2,
        (SystemConstants.VISER_VERSION_BYTES * SystemConstants.BITS_IN_BYTE - 1)) - 1) {
  ...
  public void updateWriteSiteInfo(CpuId cid, long enc, int[] si, int[] lastSi) {
    ...
    if (md.writeSiteInfo == null) {
      md.writeSiteInfo = new int[SystemConstants.LINE_SIZE()];
      md.writeLastSiteInfo = new int[SystemConstants.LINE_SIZE()];
    }
    for (int i = 0; i < SystemConstants.LINE_SIZE(); i++) {
      if (((1L << i) & enc) != 0) {
  ...
  public void updateReadSiteInfo(CpuId cid, long enc, int[] si, int[] lastSi) {
    ...
    if (md.readSiteInfo == null) {
      md.readSiteInfo = new int[SystemConstants.LINE_SIZE()];
      md.readLastSiteInfo = new int[SystemConstants.LINE_SIZE()];
    }
    for (int i = 0; i < SystemConstants.LINE_SIZE(); i++) {
      if (((1L << i) & enc) != 0) {
  ...
  /**
   * We assume that metadata is cleared at region boundaries for private cache lines. For LLC, we
  ...
  public void setAIMMD(boolean value) {
    hasAIMMD |= value;
  }
...
interface LineFactory<Line extends ViserLine> {
  ...
  public Line create(Processor<Line> proc, CacheLevel level, Line l); // Clone a line
}
