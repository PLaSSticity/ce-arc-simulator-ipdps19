...
public final class MultiProcessorTests {
	...
	public void testRead() {
		...
		// // Need to evict 64L from private L1 cache, but need to invalidate LLC cache line state
	...
	public void testWriteWriteConflict() {
		...
		System.out.println(proc0.L1cache);
		System.out.println(proc0.L3cache.getLine((new DataByteAddress(80L).lineAddress()))
	...
	public void testReadValidationConflict0() {
		...
		// Read validation of the read from 64L should fail since the value written by P0 is
		// different
		assertEquals(1, proc1.stats.pc_FailedValidations.get(), 0);

		// Due to read validation from Core 1, Core 0's deferred write should now be visible
	...
	// A read-only line should still be invalidated if it has untouched offsets and might have been
	// written by the llc.
	@Test
	 ...
	 * since region boundaries in RCC are not exactly where locks are acquired/released. T0 T1 wr/rd
	 * x ----- rel() ----- ----- acq() wr x ----- rel() acq() rd x ----- rel() acq() and rel()
	 * denote where a lock is actually acquired and released, respectively. False data race will be
	 * reported between T0's write and T1's second read although they are well synchronized.
	 ...
	 * machine.processRegionBoundary(P1, T1, EventType.LOCK_ACQUIRE); machine.cacheWrite(P0, 64L, 2,
	 * 992, T0); machine.processRegionBoundary(P0, T0, EventType.LOCK_RELEASE);
	 * machine.cacheRead(P1, 64, 2, 990, T1); machine.processRegionBoundary(P1, T1,
	 * EventType.LOCK_RELEASE); assertEquals(0, proc1.stats.pc_PreciseConflicts.get(), 0); }
	 ...
	 * machine.getProc(P1); machine.cacheRead(P1, 65L, 1, -6138712L, T1); machine.cacheWrite(P0,
	 * 64L, 2, 664, T0); machine.processRegionBoundary(P0, T0, EventType.LOCK_ACQUIRE);
	 * machine.processRegionBoundary(P1, T1, EventType.LOCK_ACQUIRE); assertEquals(1,
	 * proc1.stats.pc_PreciseConflicts.get(), 0); }
	 * @Test public void testReadValidationConflict3() { Processor<ViserLine> proc1 =
	 * machine.getProc(P1); machine.cacheWrite(P0, 65L, 2, 0, T0); machine.processRegionBoundary(P0,
	 * T0, EventType.LOCK_ACQUIRE); // Write a new value 0 to overwrite the initial value -6138712.
	 * // Otherwise, no false positive even if I don't fix the set-value-only-to-the-first-offset
	 * bug, // because of my previous fix that the simulator only reports read validation failure //
	 * at non-initial values (testReadValidationConflict1). machine.cacheRead(P1, 65L, 1, 664, T1);
	 * machine.cacheWrite(P0, 64L, 2, 664, T0); machine.processRegionBoundary(P0, T0,
	 * EventType.LOCK_ACQUIRE); machine.processRegionBoundary(P1, T1, EventType.LOCK_ACQUIRE);
	 * assertEquals(0, proc1.stats.pc_PreciseConflicts.get(), 0); }
	 */
	@Test
	public void testWriteReadConflictViaMemory() {
		...
		machine.testCacheMemoryRead(P1, 64L, 2, 0, T1);
		// The conflict is detected by the LLC, and LLC is owned by proc0, so we need to be careful
		// The LLC need not eagerly check for conflicts on a fetch
	...
	public void testLLCHit() {
		...
		// fetch 0d again, from the L3. This is a miss since the LLC line was invalidated when
		// evicting the
		// private L1 line.
	...
	/*
	 * (x0 and x1 shared the same cache line.) T1 T0 wr x0 ----- rd x1 (evict) ----- wr x0 ----- **
	...
	// A shared line's epoch should be updated only after its obsolete encodings are cleared.
	/*
	 * (x0 and x1 shared the same cache line.) T1 T0 wr x0 ----- rd x1 wr x1 (evict) ----- wr x0
	 * ----- ** false data race detected on x0!
	 */
	@Test
	...
	public void testDirtyBit() {
		...
		// Two LLC to memory message, one for control and the other for data for 128L.
		// 64L stays in LLC, and 80L need not be written back since there is no read metadata
		// System.out.println(proc1.L1cache);
		// It is okay for 80L to not have updated metadata currently, since the L3 in Viser is not
		// inclusive.
		machine.dumpMachineMemory();
	...
	public void testDeferWriteBack() {
		...
		machine.testProcessRegionBoundary(P0, T0, EventType.LOCK_RELEASE);
		// The owner core for the LLC line should be set, but the value should still be zero.
		System.out.println(proc0.L1cache);
	...
	public void testL1ReadsDuringL2Validation1() {
		...
		if (proc0.params.validateL1ReadsAlongWithL2()) {
			...
			// The conflict (failed validation) is detected since the L2 line is to be evicted after
			// being
			// updated with L1. This requires validating read-only bytes (82L) from the L2 line.
			// No read-write conflict is reported since we only validate values for a line with
			// mismatch version
			// with the LLC line.
			assertEquals(1, proc0.stats.pc_FailedValidations.get(), 0);
			assertEquals(0, proc0.stats.pc_PreciseConflicts.get(), 0);
	...
	// IMP: AIM associativity is 2 and # lines are 16, and set useAIMCache() to true
	@Test
	public void testMemoryAccesses1() {
		...
		machine.testCacheMemoryWrite(P1, 128L, 2, 11, T0);
		// Line is cached in the LLC
		assertEquals(0, proc1.stats.pc_aim.pc_WriteMisses.get(), 0);
		// 80L and 128L are going to different sets in the AIM

		machine.testCacheMemoryWrite(P0, 64L, 2, 11, T0);
		// 80L is not evicted from the AIM
		assertEquals(0, proc0.stats.pc_aim.pc_WriteMisses.get(), 0);

		machine.testCacheMemoryWrite(P1, 96L, 2, 11, T0);
		// 96L evicts 64L from the AIM
		assertEquals(0, proc1.stats.pc_aim.pc_WriteMisses.get(), 0);
		assertEquals(0, proc1.stats.pc_aim.pc_LineEvictions.get(), 0);
		// System.out.println(proc1.L1cache);
		assertEquals(0, proc1.stats.pc_aim.pc_LineEvictions.get(), 0);

		// Now evict 80L from private cache of P0
		machine.testCacheMemoryWrite(P0, 112L, 2, 99, T0);
		// This causes 80L to be evicted from the LLC and AIM. 112L is brought into the AIM. After
		// that, 80L from private cache is written back to the LLC and AIM evicts 96L.
		assertEquals(0, proc0.stats.pc_aim.pc_WriteMisses.get(), 0);
		assertTrue(proc0.aimcache.junitGetLine(proc0, new DataLineAddress(80L)).hasAIMMD());
		assertEquals(0, proc0.stats.pc_aim.pc_LineEvictions.get(), 0);

		machine.cacheRead(P1, 64L, 2, 1, T1, 0, 0, MemoryAccessType.MEMORY_READ);
		// System.out.println(proc1.L1cache);

		// Line should be in LLC, evict 128L from P1. This should cause 64L to be evicted from
		// the LLC after an LLC hit. There should not be any AIM line to evict.
		assertEquals(0, proc1.stats.pc_aim.pc_LineEvictions.get(), 0);
		assertEquals(1, proc1.stats.pc_l3d.pc_ReadHits.get(), 0);
	}
	...
	public void testAIMAccesses1() {
		...
		// AIM line size is 13 bytes
		machine.cacheRead(P0, 80L, 2, 11, T0, 0, 0, MemoryAccessType.MEMORY_READ);
		assertEquals(0, proc0.stats.pc_aim.pc_ReadMisses.get(), 0);

		machine.cacheWrite(P1, 80L, 2, 11, T0, 0, 0, MemoryAccessType.MEMORY_WRITE);
		// Line is cached in the LLC
		assertEquals(0, proc1.stats.pc_aim.pc_WriteMisses.get(), 0);

		machine.cacheWrite(P0, 96L, 2, 11, T0, 0, 0, MemoryAccessType.MEMORY_WRITE);
		machine.cacheWrite(P0, 64L, 2, 11, T0, 0, 0, MemoryAccessType.MEMORY_WRITE);
		assertEquals(0, proc0.stats.pc_aim.pc_WriteMisses.get(), 0);
		// Line 80L is now in the AIM

		machine.cacheRead(P1, 48L, 2, 20, T1, 0, 0, MemoryAccessType.MEMORY_READ);
		System.out.println(proc1.L1cache);
		// Evict 80L from P1
		machine.cacheRead(P1, 64L, 2, 20, T1, 0, 0, MemoryAccessType.MEMORY_READ);
	}
	...
	public void testAIM1() {
		...
		machine.testCacheMemoryWrite(P1, 80L, 2, 0, T1); // LLC hit, this invalidates the line in P0
		assertEquals(0, proc1.stats.pc_aim.pc_WriteMisses.get(), 0);
		assertFalse(proc1.aimcache.junitGetLine(proc1, new DataLineAddress(80L)).hasAIMMD());

		machine.testCacheMemoryWrite(P1, 96L, 2, 0, T1);
		assertEquals(0, proc1.stats.pc_aim.pc_WriteMisses.get(), 0);
