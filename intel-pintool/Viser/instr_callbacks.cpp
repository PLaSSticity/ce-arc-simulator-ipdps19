...
// Functions besides the sync functions above where we don't want to instrument memory accesses and report races. Note
// that the functions should have an exit.
...
// Extra lib functions where we don't want to instrument memory accesses and report races. These functions can have no
// exit.
...
VOID instrumentInstruction(INS ins, VOID *v) {
  // Instruments memory accesses using a predicated call, i.e. the instrumentation is called iff the instruction will
  // actually be executed.

  // On the IA-32 and Intel(R) 64 architectures conditional moves and REP prefixed instructions appear as predicated
  // instructions in Pin.
  // Instrument loads using a predicated call, i.e. the call happens iff the load will be actually executed
  // (this does not matter for ia32 but arm and ipf have predicated instructions)

  // From memory/dcache.cpp

  // Instrument each memory operand. If the operand is both read and written it will be processed twice.
  // Iterating over memory operands ensures that instructions on IA-32 with two read operands (such as SCAS and CMPS)
  // are correctly handled.

  // Prefetch instructions might access addresses which are invalid.
  if ((constants::IGNORE_PREFETCH && INS_IsPrefetch(ins)) || !INS_IsStandardMemop(ins)) {
    return;
  }
  ...
  for (UINT32 memOp = 0; memOp < memOperands; memOp++) {
    if (INS_MemoryOperandIsRead(ins, memOp)) {
      INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)readAccess, IARG_THREAD_ID, IARG_MEMORYOP_EA, memOp,
                               IARG_MEMORYREAD_SIZE, IARG_BOOL, INS_IsStackRead(ins), IARG_UINT32, op, IARG_END);
    }
    
    // Note that in some architectures a single memory operand can be both read and written (for instance incl (%eax) on
    // IA-32) In that case we instrument it once for read and once for write.
    // From SimpleExamples/pinatrace.cpp which prints both the values and addresses
    // https://groups.yahoo.com/neo/groups/pinheads/conversations/topics/5953

    if (INS_MemoryOperandIsWritten(ins, memOp)) {
      INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)beforeWriteAccess, IARG_THREAD_ID, IARG_MEMORYOP_EA, memOp,
                               IARG_MEMORYWRITE_SIZE, IARG_BOOL, INS_IsStackWrite(ins), IARG_UINT32, op, IARG_END);
      //  Instrument after the write, so that we can get the updated value. This is required in Viser for value validation.
      if (INS_HasFallThrough(ins)) {
        INS_InsertPredicatedCall(ins, IPOINT_AFTER, (AFUNPTR)afterWriteAccess, IARG_THREAD_ID, IARG_MEMORYWRITE_SIZE, IARG_UINT32, op, IARG_END);
      }
      if (INS_IsBranchOrCall(ins)) {
        INS_InsertPredicatedCall(ins, IPOINT_TAKEN_BRANCH, (AFUNPTR)afterWriteAccess, IARG_THREAD_ID, IARG_MEMORYWRITE_SIZE, IARG_UINT32, op, IARG_END);
      }
    }
  }
}
...
VOID instrumentTrace(TRACE trace, VOID *v) {
  ...
  for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
    ...
    if (strstr(OPCODE_StringShort(op).c_str(), "FENCE") || op == XED_ICLASS_LFENCE || op == XED_ICLASS_MFENCE || op == XED_ICLASS_SFENCE)
    cout << "FENCE" << OPCODE_StringShort(op) << endl;
    */
    ...
    for ( ; INS_Valid(ins); ins = INS_Next(ins)) {
      ...
      if (RTN_Valid(rtn)) {
        ...
        if (isLibFunction(rtnname.c_str())) continue;
  ...
  for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec)) {
    for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn)) {
      ...
      if (strstr(rtnName, "__parsec_roi_begin")) {
        ...
        // RTN_InsertCall(rtn, IPOINT_AFTER, (AFUNPTR) afterPthreadCreate, IARG_THREAD_ID, IARG_END);
        RTN_Close(rtn);
        ...
        // Trivial instrumentation
        // RTN_InsertCall(rtn, IPOINT_AFTER, (AFUNPTR) afterLockRelease, IARG_THREAD_ID,
        //                IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_END);
        RTN_Close(rtn);
      ...
      /* for httpd */ 
        ...
        // RTN_Open(rtn);
        // RTN_InsertCall(rtn, IPOINT_BEFORE, (AFUNPTR) startFunctionCall, IARG_THREAD_ID, IARG_CONTEXT, IARG_END);
        // RTN_Close(rtn);
      }
    }
  }
