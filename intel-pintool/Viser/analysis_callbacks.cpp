...
// Decrease the depth counter twice at __pthread_rwlock_wrlock_slow to resolve
...
BOOL isInterestingSite(int16_t tid, UINT64 value, UINT16 line, UINT16 fno,
  ...
  // for bodytrack
  // return (line == 805);
  // return (/*line == 119 || line == 206 ||*/ (value <= 10 && tid == 3 && line
...
// Currently not being called, trivial instrumentation
...
VOID afterLockAcquire(THREADID tid, ADDRINT lockAddr) {
  ...
  tdata->activeAcqs++;
  /*if (tdata->activeAcqs > 1) {
    // nested critical section
    // send a release event since we don't want the backend to send back
  signals. Event e = Event::LockEvent(tid, LOCK_RELEASE, REG_END, lockAddr);
    addEvent(e);
    // Now generate two lock access events
    Event read = Event::LockAccessEvent(tid, LOCK_ACQ_READ, lockAddr);
    addEvent(read);
    Event write = Event::LockAccessEvent(tid, LOCK_ACQ_WRITE, lockAddr);
    addEvent(write);
...
VOID beforeLockRelease(THREADID tid, ADDRINT lockAddr) {
  g_Lock.lock(tid);
  assert(tid >= 0);
  // The following assertion fails for mysqld since the server program uses
  // special lock operations. assert(lockAddr > 0);
  if (isIgnored(tid)) {
    g_Lock.unlock();
    return;
  }
  ...
  addEvent(end);
  // Now generate one lock access event
  Event write = Event::LockAccessEvent(tid, LOCK_REL_WRITE, lockAddr);
  addEvent(write);
  // Event start = Event::LockEvent(tid, LOCK_RELEASE, REG_BEGIN, lockAddr);
  // addEvent(start);
  g_Lock.unlock();
  waitForBackend(end);
}
...
// Currently not being called, trivial instrumentation
// VOID afterLockRelease(THREADID tid, ADDRINT lockAddr) {
// 	g_Lock.lock(tid);
//   assert(tid >= 0);
//   assert(lockAddr > 0);
//   Event e = Event::LockEvent(tid, LOCK_RELEASE, lockAddr);
//   addEvent(e);
//   g_Lock.unlock();
//   waitForBackend(e);
// }
...
//   assert(tid >= 0);
//   assert(lockAddr > 0);
//   // Don't issue any event yet: we generate the event based on the return
//   value of trylock() g_Lock.unlock();
// }
/*
VOID afterTryLock(THREADID tid, ADDRINT outcome) {
  ...
  if (isIgnored(tid))
    return;
  Event e = Event::LockEvent(tid, LOCK_ACQUIRE);
  addEvent(e);
  // Unlike Radish, we are not tracking happens-before.
  waitForBackend(e);
}
*/
//   // FIXME: Handle nesting

//   Event e = Event::LockEvent(tid, LOCK_ACQUIRE, lockAddr);
//   addEvent(e);
//   // Now generate two lock access events
//   Event read = Event::LockAccessEvent(tid, LOCK_ACCESS_READ, lockAddr);
//   addEvent(read);
//   Event write = Event::LockAccessEvent(tid, LOCK_ACCESS_WRITE, lockAddr);
//   addEvent(write);
//   g_Lock.unlock();
//   waitForBackend(e);
// }
...
VOID threadBegin(THREADID tid, CONTEXT *ctxt, INT32 flags, VOID *v) {
  ...
  // This is a region boundary, but is called only on behalf of the newly
  // created thread. So this boundary is only for the current thread, but we
  // should also create a boundary for the parent thread (if applicable).
...
// SB: Radish uses these methods to establish happens-before edges between the
...
VOID beforePthreadCreate(THREADID tid,
  ...
  addEvent(end);
  // Event start = Event::ThreadEvent(tid, THREAD_SPAWN, REG_BEGIN);
  // addEvent(start);
  g_Lock.unlock();
}

// // No need to generate a THREAD_START event, threadBegin() already does that
// VOID afterPthreadCreate(THREADID tid) { // tid is the id of the parent thread
//   assert(tid >= 0);
// }
...
VOID beforeJoin(THREADID tid, ADDRINT thread) {
  g_Lock.lock(tid);
  assert(tid >= 0); // Should be free from data race
  if (isIgnored(tid)) {
    g_Lock.unlock();
    return;
  }
  ...
  addEvent(end);
  if (constants::DEBUG) {
    cout << "[pintool] Before join:" << tid << endl;
  }
  g_Lock.unlock();
  waitForBackend(end);
}*/
...
VOID afterJoin(THREADID tid, ADDRINT thread) {
  ...
  Event end = Event::ThreadEvent(tid, THREAD_JOIN, REG_END);
  addEvent(end);
  // Event start = Event::ThreadEvent(tid, THREAD_JOIN, REG_BEGIN);
  // addEvent(start);
  if (constants::DEBUG) {
...
VOID addEvent(Event e) {
  ...
  if (constants::USE_SYNC_CIRCULAR_BUFFER) {
    ...
    if (constants::WRITE_EVENT_FILE) {
      // Should be invoked before e.send() because the latter would endian_swap
      // m_tid
...
VOID waitForBackend(Event e) {
  if (lockstep) {
    ...
    // swapping. sleep(2); // Sleep for some time to help with debugging
