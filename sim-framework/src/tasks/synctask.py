...
class SyncTask(Constants):
    ...
    def __syncMesiSim(options, str_sourceMachine, str_user):
        str_cmd = ("rsync -az --exclude=.svn " + str_user + "@" + str_sourceMachine + ":" +
                   SyncTask.MESISIM_ROOT + SyncTask.FILE_SEP + " " + SyncTask.MESISIM_ROOT +
                   SyncTask.FILE_SEP)
        if options.verbose >= 1:
    ...
    def __syncViserSim(options, str_sourceMachine, str_user):
        str_cmd = ("rsync -az --exclude=.svn " + str_user + "@" + str_sourceMachine + ":" +
                   SyncTask.VISERSIM_ROOT + SyncTask.FILE_SEP + " " + SyncTask.VISERSIM_ROOT +
                   SyncTask.FILE_SEP)
        if options.verbose >= 1:
    ...
    def __syncRCCSISim(options, str_sourceMachine, str_user):
        str_cmd = ("rsync -az --exclude=.svn " + str_user + "@" + str_sourceMachine + ":" +
                   SyncTask.RCCSISIM_ROOT + SyncTask.FILE_SEP + " " + SyncTask.RCCSISIM_ROOT +
                   SyncTask.FILE_SEP)
        if options.verbose >= 1:
    ...
    def syncOutputDir(options):
        ...
        str_cmd = ("rsync -az " + outputDir + " " + getpass.getuser() + "@" + str_resultsMachine +
                   ":" + SyncTask.EXP_OUTPUT)
        if options.printOnly:
            print(str_cmd)
            return
    ...
    def syncProductsDir(options):
        ...
        str_resultsMachine = options.config.getResultsMachine()
        str_cmd = ("rsync -az " + productDir + " " + getpass.getuser() + "@" + str_resultsMachine +
                   ":" + SyncTask.EXP_PRODUCTS)
        if options.printOnly:
            print(str_cmd)
            return
        call(str_cmd, shell=True)
