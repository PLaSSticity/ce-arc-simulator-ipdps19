...
class Options(Constants):
    ...
    def setOptions(self, di_options):
        ...
        self._tup_simulators = [x for x in self._tup_tools if x != "pintool" and x != "sniper"]
        ...
        self.generateEnergyStats = di_options["generateEnergyStats"]

        self.nonMesiSimulators = len(self.getSimulatorsTuple())
        for proj in self.getSimulatorsTuple():
            if util.isMESIConfig(proj) and util.isCEConfig(proj):
    ...
    def printOptions(self):
        ...
        print("Generate energy stats:", self.generateEnergyStats)
    ...
    # SB: LATER: I think this is no longer required.
    ...
    def processViserSim(self):
        return "viseroptregularplru" not in self._str_tools and "viser" in self._str_tools
    ...
    def processPauseSim(self):
        return ("pause" in self._str_tools or "restart" in self._str_tools or
                "viseroptregularplru" in self._str_tools)
