...
class BarGraph(JGraph, Constants):
    ...
    def __init__(self, options, outputPath, htmFile, key, rs, norm):
        super(BarGraph, self).__init__(options, outputPath, htmFile, key, rs, norm)

    def generateFile(self):
        """Create a jgraph file with the given data in the directory specified."""
        cwd = os.getcwd()
    ...
    def __startBar(self, fl_shade, fl_width, f, b_print, str_tool):
        ...
        f.write("newcurve marktype xbar cfill " + str_shade + " " + str_shade + " " + str_shade +
