...
public final class CEMultiProcessorTests {
	...
	public void testMemoryAccesses1() {
		...
		// +3 for the data + 1 for metadata for 80L (the line is still in LLC)

		machine.testCacheMemoryRead(P0, 128L, 2);
		// +3 for the data + 1 for metadata for 80L (the line is still in LLC)

		machine.testCacheMemoryRead(P1, 128L, 2);
		// Needs to evict a line from P1 due to recall, so +1 for writing back metadata
		machine.testCacheMemoryRead(P1, 64L, 2);
		// +3 for fetching new line + 1 for writing back metadata via recall
	}
	...
	public void testMemoryAccesses2() {
		...
		machine.testCacheMemoryRead(P0, 96L, 2);
		// +3 for a fetch + 1 for metadata
		machine.testCacheMemoryRead(P0, 112L, 2);
		// +3 for a fetch + 1 for metadata

		machine.testCacheMemoryRead(P0, 128L, 2);
		// This would evict a line from the LLC. +3 for a fetch + 1 for eviction from LLC (metadata
		// is already evicted from private cache)
	}
	...
	public void testMemoryAccesses3() {
		...
		// This would evict a line from the LLC and one from L2. +3 for a fetch + 2 for eviction
		// from LLC (metadata is already evicted from private cache) + 1 for L2's metadata
	}
