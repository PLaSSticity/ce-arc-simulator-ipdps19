...
// Holds access metadata that is evicted from a core
class CEGlobalTableValue {
	...
	public CEGlobalTableValue(long reads, long writes, boolean s, long regID) {
		localReads = reads;
		localWrites = writes;
		supplied = s;
		regionID = regID;
	...
	@Override
	public String toString() {
		return "Region ID: " + regionID + " Reads: " + localReads + " Writes: " + localWrites
				+ " Supplied: " + supplied;
	}
}

class CEPerLineMetadata<Line extends MESILine> {
	CEGlobalTableValue[] values; // Global table per core
