...
public class HierarchicalCache<Line extends MESILine> {
	...
	/**
	 * Summarize whether any cache line accesses with the active region was supplied to another
	 * cache. We only use the L2 cache's bit.
	 */
	boolean supplied = false;
	/**
	 * Indicate whether any line with non-null access bits was evicted. We only use the L2 cache's
	 * bit.
	 */
	...
	/**
	 * Update the incoming line in this cache. The line should already be present because of
	 * inclusivity.
	 *
	 * @param incoming
	 *                     the line being evicted from a lower-level cache
	 */
	protected void evictedFromHigherCache(Processor<Line> proc, final Line incoming) {
		...
		MESILine mesiLine = (MESILine) getLine(incoming);
		...
		switch (mesiLine.getState()) {
			case MESI_SHARED: {
				...
				for (Processor<Line> p : proc.allProcessors) {
					if (p == proc) { // Do not compare with this.processor, since L3 cache is on
						// behalf on PO.
						continue;
					}

					if (p.L1cache.searchGivenPrivateCache(incoming)
							|| p.L2cache.searchGivenPrivateCache(incoming)) {
						count++;
					}
				}
				if (count > 0) {
	...
	/**
	 * Access the line containing the given address, bringing it into the L1 cache if it was not
	 * already present there.
	 *
	 * While this function is recursing, each intermediate lines returned (if not already in the L1)
	 * has been removed from its containing cache, and is ready to be inserted into the L1.
	 *
	 * @param address
	 ...
	 *         was found and 2) holds a reference to the new line (now in the L1)
	 */
	public MemoryResponse<Line> request(Processor<Line> proc, final ByteAddress address,
			boolean read) {
		...
		for (Iterator<Line> it = set.iterator(); it.hasNext();) {
			...
			if (line.valid() && line.contains(address)) {
				...
				if (levelInHierarchy == CacheLevel.L1) {
					...
					assert line.lineAddress().get() == address.lineAddress()
							.get() : "Line addresses do not match.";
					assert line.getState() != MESIState.MESI_INVALID;
					...
					if (proc.params.conflictExceptions()) {
						// One return data message
						int sizeIncomingMemBytes = 0;
						...
						} else if (line.hasAIMMD()) {
							ret.hasAIMMd = true;
						}

						// When is the inMemory bit cleared? Possibly on an LLC eviction. It
						// probably does not matter any more since we now check for ongoing regions.
						if (line.inMemory()) {
							CEPerLineMetadata<Line> md = proc.machine.globalTable
									.get(line.lineAddress().get());
							assert md != null : "In-memory bit should have been unset.";
							for (int i = 0; i < proc.params.numProcessors(); i++) {
								// Other threads' local bits are retrieved from memory by accessing
								// the global table and the corresponding supplied bit is set
								if (i != proc.id.get()) {
									...
									if (val.regionID == p.getCurrentEpoch().getRegionId()) {
										...
										val.supplied = true;
										sizeIncomingMemBytes += (SystemConstants.CE_READ_METADATA_BYTES
						...
						// Some request from Core 1 hits in the LLC does not necessarily imply that
						// the outOfCache bit for Core 1 will be set.
						if (proc.L2cache.outOfCache) {
							...
							if (md != null) {
								...
								if (val.regionID == proc.getCurrentEpoch().getRegionId()) {
									ret.lineHit.orLocalReads(val.localReads);
									ret.lineHit.orLocalWrites(val.localWrites);
									if (val.supplied) {
										ret.lineHit.setSupplied();
									}
									sizeIncomingMemBytes += (SystemConstants.CE_READ_METADATA_BYTES
											+ SystemConstants.CE_WRITE_METADATA_BYTES);
								}
						...
						int sizeMemAccessBytes = SystemConstants.DATA_MESSAGE_CONTROL_BYTES
								+ +SystemConstants.DATA_MESSAGE_SIZE_BYTES
								+ proc.getCEMetadataLineSize();
						if (proc.params.useAIMCache()) {
							if (ret.aimHit == CacheLevel.L3) {
								// AIM hit so no messages to memory to access global table
							} else {
								if (line.inMemory() || proc.L2cache.outOfCache) {
									// One control to get the metadata in memory
									proc.updateTrafficForOneNetworkMessage(1, sizeOutgoingMemBytes,
											false, false);

									proc.updateTrafficForOneNetworkMessage(1, sizeIncomingMemBytes,
											false, false);
								}
							}
						} else {
		...
		if (nextCache != null) {
			...
			// Check global table. CE incurs LLC-to-memory even on a LLC hit. We account for the
			// traffic in the read()/write() methods.
			if (proc.params.conflictExceptions()) {
				...
				if (md != null) {
					...
					for (int i = 0; i < proc.params.numProcessors(); i++) {
						...
						// There is no need to account for extra traffic if the outOfCache bit is
						// not set. Seems unfair.
						if (p.L2cache.outOfCache) {
							if (id.equals(proc.id)) {
								...
								if (val.regionID == p.getCurrentEpoch().getRegionId()) {
									...
									numReturnBytes += (SystemConstants.CE_READ_METADATA_BYTES
											+ SystemConstants.CE_WRITE_METADATA_BYTES);
									hasMD = true;
								}
							} else {
					...
					// The bit is cleared if hasMD is false since the bit is false for memLine
					if (hasMD) {
						memLine.setAIMMD(hasMD);
					}
				}
		...
		if (levelInHierarchy == CacheLevel.L1) {
			Line toEvict = callbacks.eviction(ret.lineHit, set, levelInHierarchy,
					MRUBits[setIndex]);
			...
			if (proc.params.conflictExceptions()) {
				// If a cache line being evicted has a supplied or local bit set, the line address
				...
				if (toEvict.isSupplied() || toEvict.isRead() || toEvict.isWritten()) {
					...
					CEGlobalTableValue val = new CEGlobalTableValue(toEvict.getLocalReads(),
							toEvict.getLocalWrites(), toEvict.isSupplied(),
					...
					// One control + data message to set the metadata in memory
					int sizeOutgoingMemBytes = SystemConstants.CONTROL_MESSAGE_SIZE_BYTES
							+ SystemConstants.CE_READ_METADATA_BYTES
							+ SystemConstants.CE_WRITE_METADATA_BYTES;
					proc.updateTrafficForOneNetworkMessage(1, sizeOutgoingMemBytes, false, false);
					...
					// The valid check is important, since the L3 might have recalled the line
					// making it invalid. testAIM1() in CEUniAIMTests is a use case.
					// For example, P0 might read, P1 writes making P0's line
					// invalid. P0 then evicts the invalid line but it has valid metadata.
					// Maybe I need to fix when a line is set to invalid in MESI. This requires
					// understanding the difference between the invalid bit and the INVALID state.
					Line llcLine = proc.L3cache.getLine(toEvict);
					if (/* toEvict.valid() */ llcLine != null) {
						llcLine.setInMemoryBit();
						llcLine.setAIMMD(true);

						// One control + data message to set the metadata in memory
						int sizeOutgoingTrafficBytes = SystemConstants.DATA_MESSAGE_CONTROL_BYTES
								+ SystemConstants.CE_READ_METADATA_BYTES
								+ SystemConstants.CE_WRITE_METADATA_BYTES;
						sizeOutgoingMemBytes = SystemConstants.DATA_MESSAGE_CONTROL_BYTES
								+ proc.getCEMetadataLineSize();
						if (proc.params.useAIMCache()) {
							// Check if toEvict is in the AIM
							AIMResponse<Line> aimresp = proc.aimcache.request(proc, llcLine, false);
							if (aimresp.whereHit == CacheLevel.MEMORY) {
								proc.updateTrafficForOneNetworkMessage(1, sizeOutgoingTrafficBytes,
										false, false);
							}
						} else {
						}
			...
			// If we are evicting a line from a lower-level cache, then we should also evict the
			// line from all higher level caches
			...
			boolean dirtyPrivateLine = false;
			if (toEvict.valid() && toEvict.getState() != MESIState.MESI_INVALID) {
				RecallResponse resp = recallFromHigherLevelPrivateCaches(toEvict);
				...
				// eviction would mean that the L1/L2 lines are already invalid. So we need not
				// worry about counting twice.

				// Recall would include a control message + ACK, multiplied by the number of cores
				// participating
				for (int i = 0; i < resp.numInvalidations; i++) {
					int sizeBytesOutgoing = SystemConstants.CONTROL_MESSAGE_SIZE_BYTES;
					proc.updateTrafficForOneNetworkMessage(1, sizeBytesOutgoing, true, true);

					int sizeBytesReturn = SystemConstants.CONTROL_MESSAGE_SIZE_BYTES;
					proc.updateTrafficForOneNetworkMessage(1, sizeBytesReturn, true, true);
				}

				if (resp.dirty) {
					// dirty should mean one data message from the core to the LLC
					int sizeBytes = SystemConstants.DATA_MESSAGE_CONTROL_BYTES
							+ SystemConstants.DATA_MESSAGE_SIZE_BYTES;
					proc.updateTrafficForOneNetworkMessage(1, sizeBytes, true, false);
					dirtyPrivateLine = true;
				}
			}

			// write back dirty line to memory
			if (toEvict.dirty() || dirtyPrivateLine) {
				int sizeBytes = SystemConstants.TAG_BYTES + SystemConstants.DATA_MESSAGE_SIZE_BYTES;
				proc.updateTrafficForOneNetworkMessage(1, sizeBytes, false, false);
			}
			if (proc.params.usePLRU()) {
				int lineIndex = set.indexOf(toEvict);
				set.set(lineIndex, ret.lineHit);
				setMRUBit(ret.lineHit);
			} else {
				boolean found = set.remove(toEvict);
				assert found;
			...
			// It does not matter whether memLine or ret.lineHit has AIM md. We
			// need to evict a line.
			if (proc.params.useAIMCache() && toEvict.valid()
			/* && proc.needToCheckAIMCache(toEvict) */
			/* && ret.lineHit.hasAIMMD() */) {
				proc.aimcache.evictLine(proc, toEvict);
			}
			// So we just fetched a line into the LLC, create one in the AIM cache.
			if (proc.params.useAIMCache()) {
				proc.aimcache.addLineFromLLCMiss(proc, ret.lineHit, read);
			}
		} else {
	...
	/**
	 * Iterate over all processors and remove the line {@code toEvict} from all private L1/L2
	 * caches. This method is called from L3.
	 */
	...
	private RecallResponse recallFromHigherLevelPrivateCaches(Line toEvict) {
		...
		for (Processor<Line> p : processor.allProcessors) {
			...
			for (Iterator<Line> it = set.iterator(); it.hasNext();) {
				...
				if (line.valid() && line.lineAddress().get() == toEvict.lineAddress().get()) {
					...
					if (p.params.conflictExceptions()) {
						...
						// address is saved in the local table and local access/supplied bits are
						// saved in the global table.
						if (line.isSupplied() || line.isRead() || line.isWritten()) {
							...
							// if (line.isRead()) {
							// numBytes += MemorySystemConstants.CE_READ_METADATA_BYTES;
							// }
							// if (line.isWritten()) {
							// numBytes += MemorySystemConstants.CE_WRITE_METADATA_BYTES;
							// }

							if (processor.params.useAIMCache()) {
								// Check if the line is in the AIM, in that case, CE can avoid a
								// memory message
								AIMResponse<Line> aimResp = processor.aimcache.request(processor,
										toEvict, false);
								if (aimResp.whereHit != CacheLevel.L3) {
									touchedMem = true;
									numBytes += (SystemConstants.CE_READ_METADATA_BYTES
											+ SystemConstants.CE_WRITE_METADATA_BYTES);
								}
							} else {
								touchedMem = true;
			...
			// For this processor p, line toEvict was present in the L1, so it should be present in
			// L2 as well. Even otherwise, we need to check the L2.
						...
						if (line.isSupplied() || line.isRead() || line.isWritten()) {
							...
							if (foundL1) {
								...
								if (processor.params.useAIMCache()) {
									// Check if the line is in the AIM, in that case, CE can avoid a
									// memory message
									AIMResponse<Line> aimResp = processor.aimcache
											.request(processor, toEvict, false);
									if (aimResp.whereHit != CacheLevel.L3) {
										touchedMem = true;
										numBytes += (SystemConstants.CE_READ_METADATA_BYTES
												+ SystemConstants.CE_WRITE_METADATA_BYTES);
			...
			if (p.params.conflictExceptions()) {
				if (touchedMem) {
					p.updateTrafficForOneNetworkMessage(1, numBytes, false, false);
				}
			}
		}
	...
	/**
	 * Called on behalf of an L2 cache, when a valid line is being evicted. Note that the line may
	 * not be present in the L1, since the L1 could be smaller.
	 */
	@SuppressWarnings("unchecked")
	private void recallFromL1Cache(Processor<Line> proc, Line toEvict) {
		assert levelInHierarchy == CacheLevel.L2 && toEvict.valid();

		LinkedList<Line> set = processor.L1cache.sets
				.get(processor.L1cache.index(toEvict.lineAddress().get()));
	...
	public MemoryResponse<Line> ceSearchPrivateCaches(final DataByteAddress address,
			final boolean reorderSet) {
		assert processor.params.conflictExceptions();
		return __ceSearchPrivateCaches(address, reorderSet);
	}

	private MemoryResponse<Line> __ceSearchPrivateCaches(final ByteAddress address,
			final boolean reorderSet) {
		MemoryResponse<Line> ret = new MemoryResponse<Line>();
		final LinkedList<Line> set = sets.get(index(address.get()));
	...
	public void setMRUBit(Line cacheLine) {
		...
		/*
		 * if (levelInHierarchy == CacheLevel.L1) { // The corresponding L2 MRU bits should also be
		 * taken care of. nextCache.setMRUBit(nextCache.getLine(cacheLine), read); }
		 */
	}
